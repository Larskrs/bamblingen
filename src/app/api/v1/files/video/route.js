...
export async function GET(req, ctx) {
    ...
    let segmentId = req.nextUrl.searchParams.get("s");
    // segmentId = segmentId.replace(/\D/g, '');
    ...
    const _rawPath = dbFile.address
    
    let dirPath = ""
    let filePath = ""

    if (segmentId) {
        dirPath = path.posix.join(process.cwd(),`files`, `batch-${batchId}`, "_video", videoId, "stream")
        filePath = path.posix.join(dirPath, `${segmentId}`)
    } else {
        dirPath = path.posix.join(process.cwd(),`files`, `batch-${batchId}`, "_video", videoId, "stream")
        filePath = path.posix.join(dirPath, "playlist.m3u8")
    }
    
    
    const [id, extension] = videoId.split(".");
    const mimeType = mime.lookup("playlist.m3u8") || 'application/octet-stream';
